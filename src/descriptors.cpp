...
DescriptorSetLayout::DescriptorSetLayout(
        Device &deviceRef, std::unordered_map<uint32_t, VkDescriptorSetLayoutBinding> bindings)
        : device{deviceRef}, bindings{bindings} {
...
bool DescriptorPool::allocateDescriptor(const VkDescriptorSetLayout descriptorSetLayout, VkDescriptorSet &descriptor) const {
    ...
    if (vkAllocateDescriptorSets(device.getLogicalDevice(), &allocInfo, &descriptor) != VK_SUCCESS) {
    return false;
    }
    return true;
}
